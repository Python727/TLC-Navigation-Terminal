...
class TlcMap extends HTMLElement {
    ...
    el.innerHTML = `
      <div class="node-circle" style="background:none;border:none;">
        <img src="${iconPath}" class="node-icon"
             ...
             draggable="false" alt="">
        ${!visited ? `<div class="node-fog" title="Unbesucht">
  ...
  _importData(jsonStr) {
    ...
    if (!data.version) { window.tlcNotify('Unbekanntes Format', 'error'); return; }
    const s = window.TLC_STATE;
  ...
  _openNodeInfoModal(loc, focusColor = false) {
    ...
    const RES_TYPES = [
      ...
      { key: 'water',   label: 'Frischwasser',   unit: 'L',   cap: 2000,  icon: '💧' },
    ];
    ...
    const resHTML = () => (nd.resources || []).map((r, i) => `
      ...
      </div>`).join('') || `<div class="empty-hint">No entries</div>`;
    ...
    window.TLC_MODAL.open({
      ...
      body: `
        ...
        ">
          ...
          <div>
            <div style="font-size:11px;color:#00a8e8;letter-spacing:0.5px;">Unterwasser-Loot vorhanden</div>
            <div style="font-size:10px;color:#4a7a8a;margin-top:2px;">Dieser Standort enthält zugängliche Loot-Punkte unter Wasser.</div>
          </div>
        </div>` : ''}

        <!-- Tab Bar -->
        <div class="tab-bar">
          <button class="tab-btn active" data-tab="info">Farbe</button>
          <button class="tab-btn" data-tab="notes">Notizen</button>
          <button class="tab-btn" data-tab="calc">Ressourcen</button>
          <button class="tab-btn" data-tab="buildings">Gebäude</button>
          <button class="tab-btn" data-tab="enemies">Gegner</button>
        </div>
